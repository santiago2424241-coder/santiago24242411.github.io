...
// ===== PARTÍCULAS 3D AVANZADAS (REDUCIDAS Y OPTIMIZADAS) =====
function createParticles() {
    // ✅ DESACTIVAR en móvil completamente
    if (Utils.isMobile() || !CONFIG.effects.particles || !CONFIG.effects.advancedParticles) return;
    ...
    // ✅ MÁXIMO 10 partículas en desktop (antes 25)
    ...
    Utils.log('✅ Partículas 3D optimizadas (10 máximo, desactivadas en móvil)', '#198754');
}
...
// ===== SCROLL REVEAL CON LÍNEAS (MEJORADO - SIN MEMORY LEAK) =====
...
function initScrollReveal() {
    ...
    revealObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                revealObserver.unobserve(entry.target); // ✅ CLEANUP
            }
        });
    ...
    Utils.log('Scroll reveal con líneas activado (sin memory leak)', '#0d6efd');
}

// ✅ CLEANUP cuando se navega
...
// ===== EFECTOS POR SECCIÓN: REVEAL + BG + LÍNEA + PULSE (MEJORADO) =====
...
function initSectionReveal() {
    ...
    sections.forEach((section, index) => {
        ...
        sectionObservers.push(observer); // Guardar referencia
    });
    ...
    Utils.log('Efectos por sección activados (sin memory leak)', '#0d6efd');
}

// ✅ CLEANUP de todos los observers
window.addEventListener('beforeunload', () => {
